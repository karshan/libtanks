 ...
 * sets up a zero'd out Tank (generally not a good idea to do this only)
 */
Tank::Tank() : speed(0.0) {}

/**
 * Tank parametrized constructor
 * sets up the Tank from the params you passed in
 */
Tank::Tank(uint32_t id, k3d::vec2 pos, k3d::vec2 velocity, k3d::vec2 aim) :
    id(id), pos(pos), velocity(velocity), aim(aim), speed(0.0) {}
